<x-layouts.app :title="__('Gestión de Productos')">
    ...
    <div class="flex items-start justify-between">
        <div class="w-fit bg-surface rounded-md border border-gray-200 dark:border-gray-700 p-4">
            <form method="GET" action="{{ route('productos-admin.index') }}" class="flex flex-wrap items-end gap-3 md:gap-4">
                ...
                <div class="flex gap-2 pt-5">
                    <button type="submit" class="rounded-md border border-gray-300 px-3 py-1.5 text-sm font-medium text-gray-700 hover:bg-gray-100 dark:border-gray-600 dark:text-gray-300 dark:hover:bg-gray-700">Buscar</button>
        ...
        <div class="flex gap-3 items-start">
            <button onclick="abrirModalCrear()" class="inline-flex items-center gap-1 rounded-md border border-brand px-3 py-1.5 text-sm font-medium text-brand hover:bg-brand hover:text-white transition-colors">+ Agregar Producto</button>
            <button onclick="abrirModalImportar()" class="inline-flex items-center gap-1 rounded-md border border-brand px-3 py-1.5 text-sm font-medium text-brand hover:bg-brand hover:text-white transition">Importar productos desde Excel</button>
        </div>
    </div>
            ...
            <tbody class="divide-y divide-gray-100 dark:divide-gray-800">
                @foreach ($productos as $p)
                    <tr class="hover:bg-gray-50 dark:hover:bg-gray-800/40 transition">
                        ...
                        <td class="px-4 py-2 text-sm text-gray-600 dark:text-gray-400">{{ $p->descripcion }}</td>
                        ...
                        <td class="px-4 py-2 text-center">
                            <div class="flex items-center justify-center gap-2">
                                <button onclick="abrirModalEditar({{ $p->id }}, '{{ $p->nombre }}', '{{ $p->descripcion }}', {{ $p->precio }}, {{ $p->stock }}, {{ $p->imagen_id ?? 'null' }}, {{ $p->descuento_id ?? 'null' }})" class="text-xs font-medium text-yellow-600 hover:underline">Editar</button>
                                    <span class="text-gray-400">|</span>
                                    <form method="POST" action="{{ route('productos.destroy', $p->id) }}" onsubmit="return confirm('¿Eliminar este producto?')" class="inline">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" class="text-xs font-medium text-danger hover:underline">Eliminar</button>
                                    </form>
                            </div>
                        </td>
                    </tr>
            ...
            <form method="POST" action="{{ route('productos.store') }}" class="space-y-4">
                ...
                <div class="grid md:grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <input type="number" name="stock" class="w-full rounded-md border border-gray-300 px-2 py-1 text-sm focus:border-brand focus:ring-1 focus:ring-brand dark:border-gray-600 dark:bg-gray-800 dark:text-gray-100"></div>
                    </div>
                    <div>
                    ...
                    <div class="flex justify-end gap-2 pt-2">
                        <button type="button" onclick="cerrarModalCrear()" class="rounded-md border border-gray-300 px-3 py-1.5 text-sm text-gray-700 hover:bg-gray-100 dark:border-gray-600 dark:text-gray-300 dark:hover:bg-gray-700">Cancelar</button>
                        <button type="submit" class="rounded-md bg-green-600 px-3 py-1.5 text-sm font-medium text-white hover:brightness-110">Guardar</button>
                    </div>
                </div>
            </form>
        </div>
    ...
    <div id="modalImportar" style="display:none" class="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50">
        <div class="bg-surface rounded-md border border-gray-200 dark:border-gray-700 w-full max-w-md p-6">
            ...
            <form method="POST" enctype="multipart/form-data" class="space-y-4">
                @csrf
                <input type="file" name="archivo" accept=".xlsx,.xls,.csv" required class="w-full rounded-md border border-gray-300 px-2 py-1 text-sm focus:border-brand focus:ring-1 focus:ring-brand dark:border-gray-600 dark:bg-gray-800 dark:text-gray-100">
                <div class="flex justify-end gap-2 pt-2">
                    <button type="button" onclick="cerrarModalImportar()" class="rounded-md border border-gray-300 px-3 py-1.5 text-sm text-gray-700 hover:bg-gray-100 dark:border-gray-600 dark:text-gray-300 dark:hover:bg-gray-700">Cancelar</button>
                    <button type="submit" class="rounded-md bg-brand px-3 py-1.5 text-sm font-medium text-white hover:brightness-110">Importar</button>
                </div>
            </form>
        </div>
    </div>
    <div id="modalEditar" style="display:none" class="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50">
        <div class="bg-surface rounded-md border border-gray-200 dark:border-gray-700 w-full max-w-lg p-6">
            ...
            <form id="formEditar" method="POST" class="space-y-4">
                ...
                <div class="flex justify-end gap-2 pt-2">
                    <button type="button" onclick="cerrarModalEditar()" class="rounded-md border border-gray-300 px-3 py-1.5 text-sm text-gray-700 hover:bg-gray-100 dark:border-gray-600 dark:text-gray-300 dark:hover:bg-gray-700">Cancelar</button>
                    <button type="submit" class="rounded-md bg-green-600 px-3 py-1.5 text-sm font-medium text-white hover:brightness-110">Guardar</button>
                </div>
            </form>
        </div>
